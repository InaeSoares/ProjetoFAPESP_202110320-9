...
for x in range(4):
  ...
  for y in range(10):
    ...
    plt.grid()
    plt.plot(np.arange(41), normal_train_data[0]) 
    plt.title("dados normais")
    plt.savefig("dadosNormais"+str(x)+str(y))
  
    plt.grid()
    plt.plot(np.arange(41), anomalous_train_data[0]) 
...
df.to_csv(r"C:/Users/Administrator/Downloads/ICInae/DAWOUD/Autoencoder/KddFinalPreds.csv",index=False) 